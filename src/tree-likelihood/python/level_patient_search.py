...
def psearch_leaf(
    T, idx, data_list, dbest, nearest_neighbor, noise, taow=None, nnq=None
):
    ...
    """
    res = conv_jax_apply_d1m2_to_d2m1(T, data_list[idx])
...
def search_children(
    node_list, data_refs, T, index, data_list, dbest, nn, noise, taow, nnq
):
    for index in data_refs:
...
def level_patient_search(node_list, data_list, input_list, tau=0.4, tauprops=None):
    ...
    for i, T in enumerate(input_list):
        ...
        if tauprops != None:
            if tauprops[i] > 0:
                TAU = difference(
                    T.m1, jax_apply_d1m2_to_d2m1(T, data_list[tauprops[i]]), noise
                ).real
                # print(TAU)
        taow = [TAU]
        # rq
        ...
        for c in node_list[0].children:
            ...
            while len(cq) or len(sq):
                while len(cq):
                        ...
                        for c in node_list[node_index].children:
                            ...
                            if node_list[c].data_refs != None:
                                for index in node_list[c].data_refs:
                                    psearch_leaf(
                                        T, index, data_list, dbest, nn, noise, taow, nnq
                                    )
                                if FIRST:
                                    FIRST = False
                                continue
                            ...
                            # geometry things
                            a, b = check_ctf_bound(
                                node_list[c], node_list[node_index], T, noise
                            )
                            if a:
                                cq.append((c, b))
                            else:
                    ...
                    if False and not FIRST and not FIRED:
                        FIRED = True
                        while len(cq):
                            # print("fir2e")
                    ...
                    if len(cq):
                        # depth_counter
                        if (
                            False and depth_counter > 2
                        ):  # if depth exceeds 2, start doing lexBFS
                            val = len(sq)
                            # print(cq)
                            sq.extend(
                                sorted(cq, key=sortkey)[0 : max(k, int(len(cq) / 3))]
                            )
            ...
            * (
                complex_distance(
                    difference_calculation(
                        T.m1, conv_jax_apply_d1m2_to_d2m1(T, data_list[nns[0]]), noise
                    )
                )
                ** 2
            )
