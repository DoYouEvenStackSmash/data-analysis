...
def new_search_leaf(T, idx, data_list, dbest, nearest_neighbor, noise,parent, mdist,TAU=1):
...
def new_search_node(T, node_index, node_list, data_list, irad,dbest, nn, noise,TAU, depth,mdist):
  
  
  if node_list[node_index].data_refs != None:
    for index in node_list[node_index].data_refs:
      new_search_leaf(T, index, data_list, dbest, nn, noise,node_list[node_index], mdist,TAU)
    return
  if node_list[node_index].data_refs == None and irad < mdist[0]:
      ...
      new_search_node(T,dbl[0][0],node_list, data_list, dbl[0][1], dbest,nn, noise, TAU,depth+1,mdist)
      
      z = 1
      while z < min(len(dbl),3):
        new_search_node(T,dbl[z][0],node_list, data_list, dbl[z][1], dbest,nn, noise, TAU,depth+1,mdist)
        z+=1
...
def impatient_tree_likelihood(node_list, data_list, input_list):
    ...
    for i, T in enumerate(input_list):
      ...
      for c in reversed(node_list[0].children):
        ...
        mdist = [float("inf")]
        new_search_node(T, c, node_list, data_list, init_d, dbest, nn, noise, 1,0,mdist)
        nnl.append(nn[0])
